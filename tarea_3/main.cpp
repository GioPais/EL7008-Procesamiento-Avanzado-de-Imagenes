...
Mat get_hist(Mat src, String title, bool show)
{
	...
	for (int r = 0; r < src.rows; r++) {
		for (int c = 0; c < src.cols; c++) {
			...
			hist.at<float>(0, ind) = hist.at<float>(0, ind) + 1.0 / (src.rows*src.cols);
		}
	}
...
int main(void)
{
	...
	// SVM
	Mat y_pred;
	//int maxcount[20] = { 10,15,18,20,22,50,100,500,1000,2500,4000,4500,5000,5250,5500,6000,7500,10000,100000,1000000 };
	int maxcount[5] = { 20,100,5000,10000,100000};
	//float e[5] = { 0.01,0.001,0.0001,0.00001,0.000001 };
	double c_value[8] = {0.000001,0.001,1,1000,10000,100000,1000000,10000000 };
	float e[2] = { 0.001,0.00001};
	float best_acc = 0.0;
	int best_m = 10;
	float best_e = 0.001;
	double best_C = 1.0;
	for (int k = 0; k < size(c_value); k++) {
		for (int j = 0; j < size(e); j++) {
			for (int i = 0; i < size(maxcount); i++) {
				...
				svm->setC(c_value[k]);
				//svm->setTermCriteria(TermCriteria(TermCriteria::MAX_ITER, maxcount[i], e[j])); // -----------100 0.001
				svm->setTermCriteria(TermCriteria(TermCriteria::MAX_ITER, maxcount[i], e[j])); // -----------100 0.001
				...
				cout << "SVM Accuracy: " << svm_acc * 100 << "% C: " << c_value[k] << " param: " << maxcount[i] << " y " << e[j] << endl;
				if (svm_acc > best_acc) {
					best_acc = svm_acc;
					best_m = maxcount[i];
					best_e = e[j];
	...
	cout << "------------------------------" << endl;
	cout << "Best SVM Accuracy: " << best_acc * 100 << "% C: " << best_C<< " param: " << best_m << " y " << best_e << endl;
	cout << "------------------------------" << endl;
